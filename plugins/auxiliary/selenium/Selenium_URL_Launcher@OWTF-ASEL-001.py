...
def run(PluginInfo):
    ...
    OWTFLogger.log("WARNING: This plugin requires a small selenium installation, please run '" + config.FrameworkConfigGet('INSTALL_SCRIPT') + "' if you have issues")
    Content = []
    ...
    for Args in plugin_params.GetArgs({
        ...
        URLLauncher = ServiceLocator.get_component("selenium_handler").CreateURLLauncher({'BASE_URL': Args['BASE_URL'], 'INPUT_FILE': InputFile})
        URLLauncher.Run()
    return Content
