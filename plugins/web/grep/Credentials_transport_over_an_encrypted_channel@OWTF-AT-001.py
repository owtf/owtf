...
def run(PluginInfo):
    ...
    Command, RegexpName, Matches = ServiceLocator.get_component("transaction").GrepMultiLineResponseRegexp(
        ServiceLocator.get_component("config").Get('RESPONSE_REGEXP_FOR_PASSWORDS'))
    ...
    for ID, FileMatch in Matches:
        ...
        if 'https' != Transaction.URL.split(":")[0]:
            ...
            InsecureMatches.append([ID, Transaction.URL+": "+FileMatch])
            # Need to make the unique work by URL + password
    Message = "<br /><u>Total insecure matches: "+str(len(InsecureMatches))+'</u>'
    OWTFLogger.log(Message)
    Content += Message+"<br />"
    Content += ServiceLocator.get_component("plugin_helper").DrawResponseMatchesTables(
        [Command, RegexpName, InsecureMatches],
        PluginInfo)
    return Content
        """
        return []
