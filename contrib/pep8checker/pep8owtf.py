...
class OWTFStyleChecker(pep8.Checker):
...
if __name__ == '__main__':
    ...
    def usage():
        print('Usage: python pep8owtf.py [-html] <file_or_folder_to_check>*')
    ...
    errors = 0
    exclude_dirs = ['/tests', '/coverage', '/readme']
    ...
    for target in targets:
        if isdir(target):
            ...
            for dirpath, dirnames, filenames in walk(target):
                ...
                for filename in filenames:
                    ...
                    for f in exclude_files:
                        if complete_filename.endswith(f):
                            cont = True
                    if cont:
                        continue
            ...
            for f in exclude_dirs + exclude_files:
                if f in target:
                    break
            else:
