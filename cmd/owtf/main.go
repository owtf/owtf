...
func serve(settings owtfconfig.Config) error {
	...
	collectorClient := &http.Client{Timeout: time.Duration(settings.HTTP.RequestTimeoutSeconds) * time.Second}
	if address := os.Getenv("OWTF_PLUGIN_PROXY"); address != "" {
		transport, err := plugin.NewProxyTransport(address, os.Getenv("OWTF_PLUGIN_PROXY_CA"))
		if err != nil {
			return err
		}
		collectorClient.Transport = transport
	}
	catalog.ConfigureHTTP(collectorClient, settings.HTTP.UserAgent)
