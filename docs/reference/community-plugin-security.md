...
This reference explains who can do what in the community plugin marketplace and what the marketplace does **not** protect against. Review it before changing plugin upload, approval, or runner behavior.
...
read the code, not just click a button. The current marketplace does
not provide additional runtime sandboxing such as per-plugin
containers or OS-enforced resource limits.
...
pick up newly introduced columns via
