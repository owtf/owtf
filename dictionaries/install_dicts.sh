...
(
	...
	if [ $? -eq 0 ]; then # Not installed
        ...
        for file in $(ls $DICTS_DIRECTORY/fuzzdb/fuzzdb-master/Discovery/PredictableRes/ | grep raft); do
            #cp $DICTS_DIRECTORY/fuzzdb/fuzzdb-master/Discovery/PredictableRes/$file $DICTS_DIRECTORY/restricted/raft/
            ln -s $DICTS_DIRECTORY/fuzzdb/fuzzdb-master/Discovery/PredictableRes/$file $DICTS_DIRECTORY/restricted/raft/$file
        done
        echo "[*] Done"
    else
