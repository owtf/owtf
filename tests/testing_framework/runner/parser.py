...
class ArgumentParser:
    ...
    def __init__(self):
        ...
        self.parser = argparse.ArgumentParser(description="OWTF Testing framework Command Line Tool")

        self.parser.add_argument(
                "category", nargs="?",
                help="Category of the tests to run. The possible values are: plugin, framework and testing_framework")

        self.parser.add_argument(
                "-a", "--all", action="store_true", default=False, dest="all",
                help="This flag tells the framework to run all the tests.")

        self.parser.add_argument(
                "-m", "--modules", dest="modules", metavar="module1,module2...",
                help="Comma separated list of test classes to run (the name of the class has to be in CamelCase).")

        self.parser.add_argument(
                "-o", "--only", dest="only", metavar="file1.py,file2.py",
                help="Comma separated list of files containing tests.")
        self.parser.add_argument(
                "-c", "--with-coverage", dest="coverage", action="store_true", default=None,
                help="Tell the runner to perform test coverage analysis. Only compatible with the --all option.")

    def parse_arguments(self, arguments):
        self.parsed_arguments = self.parser.parse_args(arguments)
        valid_arguments, message = self.validate_combinations()
        if valid_arguments:
    ...
    def validate_combinations(self):
        ...
        return valid_arguments, message
