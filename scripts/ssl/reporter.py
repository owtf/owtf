...
try:
    ...
    f = open('%s_report.html' % orgName, 'w')
    f.write('<html>')
    f.write('<body>')
    ...
    f.write('<h2> Poodle TLS :  </h2>' + str(data[0]['endpoints'][0]['details']['poodleTls']) +
            ' (-3:timeout,-2:TLS not supported,-1:test failed,0:unknown,1:not vulnerable,3:vulnerable)')
    ...
    f.write('<h2> openSslCcs test : </h2>' + str(data[0]['endpoints'][0]['details']['openSslCcs']) +
            '(-1:test failed,0: unknown,1: not vulnerable,2:possibly vulnerbale,3:vulnerable and exploitable)')
    ...
    for chainCert in data[0]['endpoints'][0]['details']['chain']['certs']:
        f.write('<h2> Chain Cert issue: </h2>' + str(chainCert['issues']) + '(1:certificate not yet valid' +
                ',2:certificate expired,4:weak key,8:weak signature,16:blacklisted) ')
    ...
    f.write('<h2> Forward secrecy support: </h2>' +
            str(data[0]['endpoints'][0]['details']['forwardSecrecy']) +
            '(1:et if at least one browser from our simulations negotiated a Forward Secrecy suite,2:et based on' +
            'Simulator results if FS is achieved with modern clients. For example, the server supports ECDHE suites,' +
            ' but not DHE,4:set if all simulated clients achieve FS. In other words, this requires an ECDHE +' +
            ' DHE combination to be supported.')
