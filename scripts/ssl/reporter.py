...
try:
    with open(org_name+"_report.html", 'w') as f:
        ...
        for protocol in data[0]['endpoints'][0]['details']['protocols']:
            protocol_str += '<h2> Protocol SSL/TLS version supported : </h2>'+protocol['name']+'  '+protocol['version']
        ...
        for chainCert in data[0]['endpoints'][0]['details']['chain']['certs']:
            cert_chains += '<h2> Chain Cert issue: </h2>'+str(chainCert['issues']) +'(1:certificate not yet valid,2:certificate expired,4:weak key,8:weak signature,16:blacklisted)'
