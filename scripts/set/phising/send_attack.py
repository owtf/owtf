...
def Usage(Message):
    print "Usage:"
    print GetName()+" EMAIL_TARGET=? EMAIL_FROM=? SMTP_LOGIN=? SMTP_PASS=?" + \
...
ShellExec(Params['SET_DEBUG_AUTOMATE']+" "+Params['TOOL_SET_DIR']+" "+Params['SET_TMP_SCRIPT'])

# Step 4 - Remove temporary script with hard-coded values
ShellExec("rm -f "+Params['SET_TMP_SCRIPT'])
