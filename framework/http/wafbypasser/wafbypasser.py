...
from framework.http.wafbypasser.core.response_analyzer import analyze_responses, print_request, \
    print_response, analyze_chars, analyze_encoded_chars, analyze_accepted_sources
...
class WAFBypasser:
    ...
    def start(self, args):
        ...
        # Detecting Allowed Sources
            ...
            requests = detect_accepted_sources(
                self.http_helper,
                target,
                data,
                headers,
        ...
        # HPP modes
            ...
            if sent_payloads["detected"]:
                ...
                for bad_char in sent_payloads["detected"]:
                    payloads.append("%s%s" % (bad_char, good_char))
                requests = self.pm.transformed_http_requests(
                    self.http_helper,
                    methods,
                    target,
                ...
                payloads = []
                for bad_char in sent_payloads["detected"]:
                    payloads.append("%s%s" % (good_char, bad_char))
                requests = self.pm.transformed_http_requests(
                    self.http_helper,
                    methods,
                    target,
                ...
                payloads = []
                for bad_char in sent_payloads["detected"]:
                    payloads.append("%s%s%s" % (good_char, bad_char, good_char))
                requests = self.pm.transformed_http_requests(
                    self.http_helper,
                    methods,
                    target,
        ...
        # Fuzzing mode
            ...
            if fuzzing_placeholders > 1:
                Error(self.owtf,
                      "Multiple fuzzing placeholder signatures found. Only one fuzzing placeholder is supported.")
