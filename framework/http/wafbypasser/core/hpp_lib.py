...
# HPP Functions
def asp_hpp(http_helper, methods, payloads, param_name, source, url, headers, body=None):
    requests = []
    if "URL" in source.upper():
        for payload in payloads:
            new_url = asp_url_hpp(url, param_name, payload)
            for method in methods:
                requests.append(http_helper.create_http_request(method, new_url, body, headers, payload))
    elif "DATA" in source.upper():
        for payload in payloads:
            new_body = asp_post_hpp(body, param_name, payload)
            for method in methods:
                requests.append(http_helper.create_http_request(method, url, new_body, headers, payload))
    elif "COOKIE" in source.upper():
        for payload in payloads:
            new_headers = asp_cookie_hpp(headers, param_name, payload)
            for method in methods:
                requests.append(http_helper.create_http_request(method, url, body, new_headers, payload))
    return requests
...
def asp_url_hpp(url, param_name, payload):
    ...
    for pay_token in payload.split(","):
        url += "%s%s=%s" % (sep, param_name, pay_token)
        sep = '&'
    return url
...
def asp_post_hpp(body, param_name, payload):
    if body is None or body == '':
        ...
        sep = '&'
    for pay_token in payload.split(","):
        body += "%s%s=%s" % (sep, param_name, pay_token)
        sep = '&'
    return body
...
def asp_cookie_hpp(headers, param_name, payload):
    ...
    try:
        ...
        sep = ""
    for pay_token in payload.split(","):
        cookie_value += "%s%s=%s" % (sep, param_name, pay_token)
        sep = '&'
    new_headers.add("Cookie", cookie_value)
    return new_headers


def param_overwrite(http_helper, param, accepted_source, payload, url, body, headers):
    requests = []
    if "URL" == accepted_source:
        ...
        requests.append(http_helper.create_http_request("GET", new_url, new_body, headers, payload))

        requests.append(http_helper.create_http_request(accepted_source, new_url, body, new_headers, payload))
        # Duplicate param , first is empty and the second contains the payload
        new_url = HTTPHelper.add_url_param(new_url, param, payload)
        requests.append(http_helper.create_http_request(accepted_source, new_url, body, headers, payload))
        return requests
        ...
        requests.append(http_helper.create_http_request(accepted_source, url, new_body, new_headers, payload))
        # Overwrite body with url
        requests.append(http_helper.create_http_request(accepted_source, new_url, body, headers, payload))
        # Duplicate param , first is empty and the second contains the payload
        new_body = HTTPHelper.add_url_param(new_body, param, payload)
        requests.append(http_helper.create_http_request(accepted_source, url, new_body, headers, payload))
        return requests
        ...
        requests.append(http_helper.create_http_request(accepted_source, url, body, new_headers, payload))
        return requests
