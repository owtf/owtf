...
def get_args():
        ...
        parser.add_argument(
            ...
            action='store',
            nargs="+",
            help="Specify Method . (Ex: -X GET . The option @@@all@@@ loads all the HTTP methods"
                 "which are listed in ./payload/HTTP/methods.txt).Custom methods can be defined in this file."
        )
        ...
        parser.add_argument("-t", "--target", dest="TARGET", action='store', required=True, help="The target url")
        ...
        parser.add_argument(
            ...
            action='store',
            type=int,
            help="Changes the Fuzzing method from asynchronous to synchronous(slower). This Allows you to follow "
                 "cookies and specify a delay time in seconds before sending a request."
        )
        parser.add_argument(
            "-fc", "--follow-cookies",
            dest="FOLLOW_COOKIES",
        ...
        parser.add_argument(
            ...
            dest="MODE",
            required=True,
            choices=['fuzz', 'detect_chars', 'asp_hpp', 'param_overwriting', "length", "detect_accepted_sources",
                     "content_type_tamper", "show_transform_functions", "overchar"],
            action='store',
            help="Select mode:(fuzz)Fuzzing mode.\n(detect_chars)Detects the available characters and attempts to find"
                 " bypasses.\n(asp_hpp)Splits the payload to comma (,) character and sends using HPP\n"
                 "(param_overwriting)Overwrites a parameter by using HPP\n"
                 "(length)Detects the length of a content placeholder\n"
                 "(detect_accepted_sources)Detected the accepted sources of a parameter\n"
                 "(content_type_tamper)Content type tampering is changing the Content-Type header and"
                 " tries to detect anomalies.\n"
                 "(show_tranform_function) Shows transformation functions\n"
