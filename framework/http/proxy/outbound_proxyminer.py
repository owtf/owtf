...
class Proxy_Miner:
    ...
    def start_miner(self):
        ...
        for i in enumerate(Websites):
            print "[#] Fetching data from : %s" % Websites[i][1]
            try:
                Proxies += Websites[i][0]()
            except Exception as error:
                print """
                    Please Report this information to OWTF community!!!\n
                    Mining Proxies Data from : %s Failed!\n
                    *****************************************************\n
                                         Exception                     \n
                    %s\nFetching data from %s  skipped\n
                    *****************************************************\n
                    """ % (Websites[i][1], str(error), Websites[i][1])
        print "[#] Fetching Done"
...
# ----------------------------------HELPER-FUNCTIONS----------------------------
    ...
    def check_proxy(self, proxy):
        ValidIpAddressRegex = """
            ...
            (25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9]?[0-9])\.){2}(25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9]?[1-9])
        """
        try:
            port = int(proxy[1])
        except:
...
# Mining Function returns list of proxies.
    ...
    def proxylisty(self):
        ...
        for i in range(1, 14):  # Fetch 13 pages
            page = BeautifulSoup(self.getpage_with_user_agent("http://www.proxylisty.com/ip-proxylist-%s" % str(i)))
