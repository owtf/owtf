...
ALLOWED_TAGS = ('html', 'body', 'a', 'p', 'h1', 'h2', 'h3', 'h4', 'div', 'span', 'i', 'b', 'u', 'table', 'tbody',
                'tr', 'td', 'th', 'strong', 'em', 'sup', 'sub', 'ul', 'ol', 'li')
...
if 'sanitiser.py' in sys.argv[0]:  # When called as a script run tests
    ...
    Sanitiser.TestPrint("Filter 4 = cleanURLs(clean_html(white_list)) = ",
                        Sanitiser.CleanURLs(clean_html(Sanitiser.Cleaner.clean_html(Input))))
    Sanitiser.TestPrint("Filter 5 = cleanURLs(white_list(clean_html)) = ",
                        Sanitiser.CleanURLs(Sanitiser.Cleaner.clean_html(clean_html(Input))))
    Sanitiser.TestPrint("Filter 6 = white_list(clean_html(cleanURLs)) = ",
                        Sanitiser.Cleaner.clean_html(clean_html(Sanitiser.CleanURLs(Input))))
