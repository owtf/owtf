...
class InterfaceServer(OWTFProcess, BaseComponent):
    def pseudo_run(self):
        ...
        try:
            self.server.bind(
                int(config.FrameworkConfigGet("UI_SERVER_PORT")),
                address=config.FrameworkConfigGet("SERVER_ADDR")
            )
            tornado.options.parse_command_line(args=[
                'dummy_arg',
                '--log_file_prefix=%s' % db_config.Get('UI_SERVER_LOG'),
                '--logging=info']
            )
            self.server.start(0)
            db.create_session()
...
class FileServer(BaseComponent):
    ...
    def start(self):
        try:
            ...
            self.server.bind(
                int(config.FrameworkConfigGet("FILE_SERVER_PORT")),
                address=config.FrameworkConfigGet("SERVER_ADDR")
            )
            tornado.options.parse_command_line(args=[
                'dummy_arg',
                '--log_file_prefix=%s' % db.Config.Get('FILE_SERVER_LOG'),
                '--logging=info']
            )
            self.server.start(1)
