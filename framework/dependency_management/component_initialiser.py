...
class ComponentInitialiser():
    ...
    def step_1(owtf_pid, root_dir):
        DB(None)
        config = Config(root_dir, owtf_pid, None)
        db_config = ConfigDB(None)
        CommandRegister(None)
        TargetDB(None)
        ResourceDB(None)
        ErrorDB(None)
        ErrorHandler()
    ...
    @staticmethod
    def step_2(args, owtf_pid, root_dir):
        PluginHandler(None, args)
        Reporter(None)
        POutputDB(None)
        ServiceLocator.get_component("db").Init()
        Shell(None)
        PluginParams(None, args)
        SMTP(None)
        #DebugDB()
        ZAP_API(None)
