...
data = None
try:
    # Opening the json document generated by ssl-labs
...
try:
    with open(org_name + "_report.html", "w") as f:
        ...
        insecureRenegotiationSuported = (
            "true"
            if data[0]["endpoints"][0]["details"]["renegSupport"] == 1
            else "false"
        )
        ...
        if "dhUsesKnownPrimes" in data[0]["endpoints"][0]["details"]:
            insecureDH = (
                "true"
                if data[0]["endpoints"][0]["details"]["dhUsesKnownPrimes"] == 2
                else "false"
            )
        ...
        for protocol in data[0]["endpoints"][0]["details"]["protocols"]:
            protocol_str += (
                "<h2> Protocol SSL/TLS version supported : </h2>"
                + protocol["name"]
                + "  "
                + protocol["version"]
            )
        ...
        certificate_exp = (
            "true"
            if data[0]["endpoints"][0]["details"]["chain"]["issues"] == 2
            else "false"
        )
        ...
        selfSigned = (
            "true"
            if data[0]["endpoints"][0]["details"]["cert"]["issues"] == 64
            else "false"
        )
        ...
        secureRenegotiationSuported = (
            "true"
            if data[0]["endpoints"][0]["details"]["renegSupport"] == 2
            else "false"
        )
