...
def GetParams():  # Basic validation and parameter retrieval:
    MandatoryParams = [
        'EMAIL_TARGET', 'EMAIL_FROM', 'SMTP_LOGIN', 'SMTP_PASS', 'SMTP_HOST', 'SMTP_PORT', 'EMAIL_PRIORITY',
        'PDF_TEMPLATE', 'MSF_LISTENER_PORT', 'MSF_LISTENER_SETUP', 'ATTACHMENT_NAME', 'SET_EMAIL_TEMPLATE',
        'PHISHING_PAYLOAD', 'PHISHING_SCRIPT_DIR', 'TOOL_SET_DIR'
    ]
...
with open(Params['SET_TMP_SCRIPT'],
          'w') as file:  # Step 2 - Create temporary script with hard-coded values from parameters:
    file.write(multi_replace(open(Params['SET_PARAMS_SCRIPT']).read(), Params))

ShellExec(Params['SET_DEBUG_AUTOMATE'] + " " + Params['TOOL_SET_DIR'] + " " +
          Params['SET_TMP_SCRIPT'])  # Step 3 - Run SET script
