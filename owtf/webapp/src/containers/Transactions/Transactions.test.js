...
import {
  transactionLoadReducer,
  transactionsLoadReducer,
  hrtResponseLoadReducer
} from "./reducer";
...
describe("Transactions Page componemt", () => {
  describe("Testing dumb Transactions component", () => {
    ...
    it("Should pass correct props to its child components", () => {
      ...
      expect(transactiontable.props().target_id).toEqual(
        wrapper.instance().state.target_id
      );
      expect(transactionheaders.props().target_id).toEqual(
        wrapper.instance().state.target_id
      );
    });
  });
  ...
  describe("Testing connected Transactions page component", () => {
    ...
    it("Props should match the initial state", () => {
      ...
      expect(wrapper.props().transactionsLoading).toEqual(
        transactionsLoadingProp
      );
      expect(wrapper.props().transactionsError).toEqual(transactionsErrorProp);
      expect(wrapper.props().transactionLoading).toEqual(
        transactionLoadingProp
      );
  ...
  describe("Testing TargetList component", () => {
    ...
    it("Should correctly render its sub-components", () => {
      ...
      const tablist = wrapper.find(
        ".transactionsPage__targetListContainer__headingAndList__listContainer__listWrapper"
      );
      const sidebartab = wrapper.find(
        ".transactionsPage__targetListContainer__headingAndList__listContainer__listWrapper__listItem"
      );
      ...
      expect(sidebartab.at(0).props().children).toBe(
        props.targets[0].target_url
      );
    });
  });
  ...
  describe("Testing Transaction Table component", () => {
    ...
    it("Should correctly render table's sub-components", () => {
      const searchHeader = wrapper.find(
        ".transactionsTableContainer__tableWrapper__headerContainer input"
      );
      const textHeader = wrapper.find(
        ".transactionsTableContainer__tableWrapper__headerContainer span"
      );
      const tableRow = wrapper.find(
        ".transactionsTableContainer__tableWrapper__bodyContainer__rowContainer"
      );
      const textCell = wrapper.find(
        ".transactionsTableContainer__tableWrapper__bodyContainer__rowContainer span"
      );
      ...
      expect(textCell.at(0).props().children).toEqual(
        props.transactions[0].url
      );
      expect(textCell.at(1).props().children).toEqual(
        props.transactions[0].method
      );
    });
  ...
  describe("Testing Transaction Header component", () => {
    ...
    it("Should correctly render its sub-components", () => {
      const tablist = wrapper.find(
        ".transactionsHeader__requestResponseHeaderToggle"
      );
      const tabs = wrapper.find(
        ".transactionsHeader__requestResponseHeaderToggle span"
      );
  ...
  describe("Testing the sagas", () => {
    describe("Testing getTransactions saga", () => {
      ...
      it("Should handle transactions load errors in case of failure", async () => {
        ...
        expect(api.getTransactionsAPI.mock.calls.length).toBe(1);
        expect(dispatchedActions).toContainEqual(
          transactionsLoadingError(error)
        );
      });
    });
    ...
    describe("Testing getHrtResponse saga", () => {
      ...
      it("Should fetch HRT response in case of success", async () => {
        ...
        api.getHrtResponseAPI = jest.fn(() =>
          jest.fn(() => Promise.resolve(mockedHrtResponse))
        );
        ...
        expect(dispatchedActions).toContainEqual(
          hrtResponseLoaded(mockedHrtResponse)
        );
      });
