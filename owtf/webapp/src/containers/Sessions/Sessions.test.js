...
describe("Sessions componemt", () => {
  describe("Testing dumb sessions component", () => {
    ...
    it("Should correctly render its sub-components", () => {
      ...
      expect(textInput.at(1).props().placeholder).toEqual(
        "Enter new session...."
      );
    ...
    it("Should update state on TextInput change event", () => {
      const textInput = wrapper
        .find(".sessionsContainer__newSessionContainer__input")
        .at(0);
      const event = {
        preventDefault() {},
    ...
    it("Should call onCreateSession on add session button click", () => {
      ...
      const addButton = wrapper
        .find(".sessionsContainer__newSessionContainer__button")
        .at(0);
  ...
  describe("Testing Session Table component", () => {
    ...
    it("Should correctly render table's sub-components", () => {
      const searchHeader = wrapper.find(
        ".sessionTableContainer__tableHeader input"
      );
      const textHeader = wrapper.find(
        ".sessionTableContainer__tableHeader span"
      );
      const tableRow = wrapper.find(
        ".sessionTableContainer__tableBody__rowContainer"
      );
      const textCell = wrapper.find(
        ".sessionTableContainer__tableBody__rowContainer__sessionNameCell"
      );
      const iconButton = wrapper.find(
        ".sessionTableContainer__tableBody__rowContainer__deleteButtonCell button"
      );
      const radio = wrapper.find(
        ".sessionTableContainer__tableBody__rowContainer__radioCell input"
      );

      expect(searchHeader.length).toBe(1);
      expect(searchHeader.props().placeholder).toEqual(
        "Search Session name...."
      );
    ...
    it("Should filter the sessions correctly", () => {
      ...
      expect(
        wrapper.find(".sessionTableContainer__tableBody__rowContainer").length
      ).toBe(2);
  ...
  describe("Testing the sagas", () => {
    ...
    describe("Testing patchSession saga", () => {
      ...
      it("Should activate a session and load sessions in case of success", async () => {
        ...
        expect(dispatchedActions).toContainEqual(
          sessionsChanged(fakeAction.session)
        );
        expect(dispatchedActions).toContainEqual(loadSessions());
      });
